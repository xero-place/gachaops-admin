...
function OnboardWizard({ onClose, onCreated }: { onClose: () => void; onCreated: () => void }) {
  ...
  const willGroup = devices.length >= 2 && masterCount === 1;
  ...
  async function submit() {
    ...
    try {
      const payload = {
        ...
        // Step2(店舗)は廃止。会社名の既定店舗を1つ自動作成（顧客が後から本当の店舗を追加）。
        stores: [{
          id: undefined,
          name: custName.trim(),
          address: '（未設定）',
...
function EditCustomerDialog({
  ...
  const dirty = name.trim() !== target.name;
  const canSave = name.trim().length > 0 && dirty && !submitting;

  async function save() {
    setSubmitting(true);
    setError(null);
    try {
      await api.put(`/customers/${target.id}`, { name: name.trim() });
      onSaved();
    } catch (e) {
